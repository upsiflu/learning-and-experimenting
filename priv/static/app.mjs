...
function width(value) {
  return property2("width", int3(value));
}
...
function view_cat(cat) {
  return img(toList([src(cat.url), width(400)]));
}
function view(model) {
  return div(
    toList([]),
    toList([
      div(
        ...
        toList([
          ...
          button(
            toList([
              on_click(new UserClickedRemoveCat()),
              disabled(model.total < 1)
            ]),
            toList([text3("Remove cat")])
          )
        ])
      ),
      div(toList([]), map(model.cats, view_cat))
    ])
  );
}
function main() {
